"""Genetic algorithm for UAV path planning."""
...
from mutapp.config import MapConfig
...
class GeneticAlgorithm:
    def __init__(
        ...
        parents_selection: str = "Random",
        tournament_size: int = 5,
        mutation_selection: str = "Worst",
        survivors_selection: str = "Fitness",
    ):
        self.system = system
        self.map_cfg = map_cfg
        self.n_uavs = len(system.list_of_UAVs)
        self.n_pop = n_pop
    ...
    def initRandomPopulation(self):
        ...
        for _ in range(self.n_pop):
            paths = self.system.init_random_path_solution(
                self.map_cfg.x_map, self.map_cfg.y_map, self.map_cfg.no_path_points
            )
            self.system.list_of_UAVs = paths
            new_sol = copy.deepcopy(self.system.list_of_UAVs)
            new_fvalue = System.get_fitness(new_sol, self.system.Weight, self.system)
    ...
    def uav_crossover(self, uav1:UAV, uav2:UAV):
        ...
        for point_idx in range(len(uav1.path)):
            if (point_idx + 1) % (self.map_cfg.no_path_points + 1) == 0:
    ...
    def mutate_position(self, position:Point):
        ...
        xm, ym = self.map_cfg.x_map, self.map_cfg.y_map
        return Point(
            position.x + r * (xm - position.x) + (1 - r) * (0 - position.x),
            position.y + r * (ym - position.y) + (1 - r) * (0 - position.y),
        )
    ...
    def perform_algorithm(
        self, on_generation: Optional[Callable[[int], None]] = None
    ) -> None:
        ...
        for gen_idx in range(self.max_iter):
            # print('iteration:', _)
            ...
            for i in range(0, n_crossovers*2, 2):
                ...
                new_gen_fvalues.append(System.get_fitness(ch1, self.system.Weight, self.system))
                new_gen_ages.append(0)
                # if len(new_gen_sols) == int(self.p_crossover * self.n_pop):
                #     break
                new_gen_sols.append(ch2)
                new_gen_fvalues.append(System.get_fitness(ch2, self.system.Weight, self.system))
                new_gen_ages.append(0)
            ...
            for i in range(n_mutations):
                ...
                new_gen_fvalues.append(System.get_fitness(ch, self.system.Weight, self.system))
                new_gen_ages.append(0)
            # Elite Members
            ...
            if on_generation is not None:
                on_generation(gen_idx + 1)

    @property
    def convergence_history(self) -> List[float]:
        return self.best_fvalues or []
