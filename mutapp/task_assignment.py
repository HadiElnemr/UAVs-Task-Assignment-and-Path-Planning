"""Task assignment metaheuristics: simulated annealing, genetic algorithm, ant colony."""
...
from mutapp import models

if TYPE_CHECKING:
    from mutapp.models import System
...
def is_feasible(list_of_UAVs) -> bool:
...
def simulated_annealing_task_assignment(sys: System, T0: float, Tf: float, beta: float) -> list[float]:
    sys.update_UAVs(
        models.assign_random_tasks_feasible(
            sys.list_of_tasks, copy.deepcopy(sys.list_of_UAVs)
        )
    )
    i=1
...
simulated_annealing_TaskAssignment = simulated_annealing_task_assignment
...
def generate_random_generation(sys1,number_of_population):
    ...
    for i in range(number_of_population):
        ...
        models.assign_random_tasks_feasible(sys1.list_of_tasks, uavs)
        ch=child(uavs)
        g.append(ch)
    return generation(g)
...
def ga_task_assignment(
    sys, percent_elite, percent_crossover, percent_mutant, number_of_population, number_of_generations
):
    ...
    return prev_g.list_of_children[0].list_of_UAVs, best


GA_TaskAssignment = ga_task_assignment
...
def ACO_TaskAssignment(sys,number_of_ants,number_of_iterations,initial_phermone,rho,alpha,beta,Q):
    ...
    return best_ant.list_of_UAVs, objs
